...
interface ReservationDAO {
    ...
    fun getAllByUserId(userId: String) : LiveData<List<Reservation>>

    @Transaction
    @Query("SELECT * FROM reservations WHERE date = :date and user_id = :userId")
    fun getAllByDateAndUserId(date: Date, userId: String): LiveData<List<ReservationWithCourt>>
    ...
    fun getAllBeforeMomentAndUserId(date: Date, slotNumber: Int, userId: String): LiveData<List<ReservationWithCourt>>

    @Query("SELECT slot_number FROM reservations WHERE court_id = :courtId AND date = :date AND user_id = :userId")
    fun getAllByCourtIdAndDateAndUserId(courtId: Long, date: Date, userId: String) : LiveData<List<Int>>
